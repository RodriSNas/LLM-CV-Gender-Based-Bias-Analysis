...
RAW_OUTPUT      = OUTPUT_DIR / "raw_responses_domain_analysis.jsonl"
SUMMARY_OUTPUT  = OUTPUT_DIR / "scores_summary_domain_analysis.csv"
...
class EvaluationTask:
    cv_id:           str
    version:         str   # original / male / female
    domain:          str
    technique:       str
    sample_index:    int   # 0 for single-sample; 0..N-1 for self-consistency
    jd_index:        int
    cv_text:         str
    job_description: str
...
def parse_response(response_text: str) -> tuple[float, str]:
    try:
        ...
        return -1.0, f"Parse error: {e} | Raw: {response_text[:200]}"
...
def save_raw_result(result: EvaluationResult, filepath: Path) -> None:
    """Appends a single result to the JSONL raw output file."""
