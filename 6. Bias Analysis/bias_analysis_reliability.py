...
CSV_PATH   = "C:/Users/rodri/Desktop/Mestrado/Tese/Method/results/openai/scores_summary/scores_summary_reliability_run_full_pipeline.csv"
OUTPUT_XLSX = os.path.join(os.path.dirname(CSV_PATH), "bias_analysis_reliability_run_full_pipeline.xlsx")
...
RUN_IDS = sorted(df_full["run_id"].unique())
...
for run_id in RUN_IDS:
    ...
    df_run = df_full[df_full["run_id"] == run_id].copy()
    pivot  = build_pivot(df_run)
    all_results[run_id]        = analyse_techniques(pivot)
    all_domain_results[run_id] = analyse_domains(pivot)
    all_sr_results[run_id]     = analyse_score_range(pivot)
...
c = ws_sum["A1"]
c.value = "Run-to-Run Reliability — Summary (all runs × techniques)"
c.font = _font(bold=True, color=CLR_WHITE, size=12); c.fill = _fill(CLR_HEADER); c.alignment = _align("center")

sum_headers = ["Run", "Technique", "n", "Mean Δ", "Std Δ",
               "M>F (%)", "F>M (%)", "Equal (%)",
...
for run_id in RUN_IDS:
    ...
    for tech in TECHNIQUES:
        ...
        _data_row(ws_sum, row, [
            f"Run {run_id}", f"{tech}{'  [baseline]' if tech == BASELINE else ''}",
...
for tech in TECHNIQUES:
    ...
    n_cols = 1 + len(RUN_IDS)
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=n_cols)
    c = ws.cell(row=1, column=1, value=f"{tech.upper()}{'  [BASELINE]' if is_base else ''}  —  metrics across runs")
    c.font = _font(bold=True, color=CLR_WHITE, size=12); c.fill = _fill(CLR_HEADER); c.alignment = _align("center")

    _header_row(ws, 2, ["Metric"] + [f"Run {r}" for r in RUN_IDS], bg=CLR_SECTION)

    def get_val(run_id, key):
        r    = all_results[run_id].get(tech, {})
        base = all_results[run_id].get(BASELINE, {})
...
for run_id in RUN_IDS:
    ...
    for (domain, tech), r in sorted(dr.items()):
        ...
        _data_row(ws_dom, row, [
            f"Run {run_id}", domain_label, tech, r["n"],
